...
server.post("/clases/", function (req, res, next) {
  const error = validateClase(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.titulo); // Generate a slug for new clases.
    next();
  }
});
...
function validateClase(clase) {
  if (!clase.titulo) return "Title is required.";
  if (!clase.profesorId) return "Author is required.";
  if (!clase.categoria) return "Category is required.";
  return "";
}
